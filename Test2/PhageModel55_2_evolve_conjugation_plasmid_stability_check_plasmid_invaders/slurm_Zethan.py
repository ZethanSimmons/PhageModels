...
"""
MEMO: This experiment is to check what conjugation rates spontaneously evolve, and the stability outcomes. It also runs a new experiment at the end to check whether starting with the evolved rate at t0 makes any difference. It has influx from chromosome-only and plasmid-bearing genotypes
"""


home = os.getenv("HOME")

needed_files = [home + '/Thesis/PhageModel45_2_evolve_conjugation_plasmid_stability_check_plasmid_invaders/demo', home + '/Thesis/PhageModel45_2_evolve_conjugation_plasmid_stability_check_plasmid_invaders/source.tar.gz', home + '/Thesis/PhageModel45_2_evolve_conjugation_plasmid_stability_check_plasmid_invaders/slurm_Zethan.py']

def main():
    base_dir = '/nesi/nobackup/uoa04131/data/PhageModel/PhageModel55_2_evolve_conjugation_plasmid_stability_check_plasmid_invaders/'

    command = """#!/bin/bash -e

#SBATCH --job-name=552_{pops:}_{count:}      # job name (shows up in the queue)
...
srun demo --population {pops:} --s_antibiotic_time_periodicity {ab_period:} --s_antibiotic_time_limit {ab_lim:} --s_the_seed {seed:} --a_antibiotics {antibiotics:} --a_v_resistance {vres:} --a_p_resistance {pres:} --a_b_resistance {bres:} --a_lysogenization_rate {lysg:} --a_lysogenization_rate_001 {lysg1:} --a_lysis_rate {lys:} --a_lysis_rate_001 {lys1:} --a_conjugation {conj:} --a_conjugation_010 {conj1:} --s_random_mixing {rand_mix:} --s_death_lysis_switch {dlys:} --a_b_death {deth:} --a_v_diffusion {v_diffus:} --s_run1 {run1:} --s_run2 {run2}

"""
    #Note that all influx rates have already been set in main.cpp, due to the more convenient interface there
    populations = (110, 190, 910)
    ab_treatments = ((1,10), (1, 100), (1,1000))
    ab_strengths = (0.3,)
    conjugation_rates = (0.14,)
    lysis_rates = (0.000,)
    lysogenization_rates = (0.00,)
    random_mixing_switches = (0, 1)
    death_rates = (0.02, 0.06, 0.12)
    death_lysis_switches = (0,)
    v_diffusion_rates = (0.0,)
    the_count = 0
...
def runsim(directory, command):
    ...
    try:
        subprocess.run(['/bin/cp', '-p'] + needed_files + [directory], check=True)
    except subprocess.CalledProcessError as err:
        print('ERROR:', err, file=sys.stderr)
        exit(1)
