...
def main():
    base_dir = '/nesi/nobackup/uoa04131/data/PhageModel/PhageModel55_0_evolve_lysis_lysogenization_phage_stability_check_only_phage_invaders/'

    command = """#!/bin/bash -e

#SBATCH --job-name=550_{pops:}_{count:}      # job name (shows up in the queue)
                                ...
                                for death_rate in death_rates:
                                    for death_lysis_switch in death_lysis_switches:
                                        for v_diffusion_rate in v_diffusion_rates:
                                            if (not (v_diffusion_rate == 0.18 and rmixswitch == 1)):
                                                label_count+=1
