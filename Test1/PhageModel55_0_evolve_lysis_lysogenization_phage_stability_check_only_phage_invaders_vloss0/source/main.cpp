...
// Function to scan a file and return it as a table
std::vector<std::unordered_map<std::string, std::string>> scan_file(const std::string& filename) {
...
double calculate_mean_last_n_rows(const std::vector<std::unordered_map<std::string, std::string>>& table, 
                                const std::string& column_name, 
                                size_t n_rows) {
    // Check if table is empty or n_rows is 0
    if (table.empty() || n_rows == 0) {
        std::cerr << "Error: Empty table or n_rows = 0" << std::endl;
        return 0.0;
    }

    // Adjust n_rows if it's larger than table size
    n_rows = std::min(n_rows, table.size());

    // Get starting index for last n rows
    size_t start_index = table.size() - n_rows;
    
    double sum = 0.0;
    size_t valid_values = 0;

    // Sum the values from the last n rows
    for (size_t i = start_index; i < table.size(); ++i) {
        ...
        if (row.find(column_name) != row.end()) {
            try {
                sum += std::stod(row.at(column_name));
                valid_values++;
            } catch (const std::exception& e) {
                std::cerr << "Error converting value at row " << i << ": " << e.what() << std::endl;
                continue;
            }
        }
    }

    // Return mean if we have valid values, otherwise return 0
    return valid_values > 0 ? sum / valid_values : 0.0;
}
...
{   
    ...
    s_ops["make_movie_flag"] = true; //outputs image of grid as jpgs, at chosen time interval
    s_ops["micro_movie_gap"] = 100; //outputs movies close together at the very start of the simulation
    s_ops["micro_movie_limit"] = 1000;
    s_ops["init_movie_gap"] = 2000;
    s_ops["init_movie_limit"] = 100000;
    s_ops["movie_gap"] = 1000000; //time interval to make jpg image outputs
    ...
    a_prms["conjugation_t0"] = 0.14; //conjugation rate of plasmids at time 0
    ...
    a_prms["lysogenization_rate"] = 0.85; //rate of lysogenization after infection of bacteria by phages - non-lysogenized, infected bacteria are lysed
    a_prms["lysogenization_rate_001"] = 0.85; //rate of lysogenization after infection of bacteria by 001 phages
    a_prms["lysogenization_rate_t0"] = 0.85; //rate of lysogenization after infection of bacteria by phages at time 0
    a_prms["lysis_rate"] = 0.005; //background lysis rate of lysogens
    a_prms["lysis_rate_001"] = 0.005; //background lysis rate of bacteria
    a_prms["lysis_rate_t0"] = 0.005; //background lysis rate of lysogens at time 0
    ...
    a_prms["v_loss_rate"] = 0.000; //loss rate of phages from lysogens - for simplicity, treated as a segregative loss rate, like phages, and occurs at reproduction, even though in real life viral loss can occur outside of reproduction 
    ...
    0.0, 
    0.000001, 0.00000, 0.00000, 0.000001, 0.00000, 0.00000, //no phage
    0.000001, 0.00000, 0.00000, 0.000001, 0.00000, 0.00000, //susceptible phage
    0.000001, 0.00000, 0.00000, 0.000001, 0.00000, 0.00000  //AB-resistant phage
    }; 
    ...
    std::string loc2 = output_location + "/02_experiment_run_evolved_";
    ...
    std::string output1 = loc1 + "vloss0_CA";
    std::string output2 = loc2 + "vloss0_CA";


    // Step 1: Setup first 10,000,000-timestep run with no influx, to evolve parameters
    if (s_ops["run1"])
    {
    s_ops["max_time"] = 100000001; // Set max_time for the 1,000,000-timestep run
    s_ops["evolve_parameters_switch"] = true; // Enable parameter evolution
    s_ops["output_gap"] = 200;

    // Step 2: Run the initial 1,000,000-timestep run to determine the time 0 starting layout
    std::cout << "Start initial 1,000,000-timestep run\n";
    std::vector<unsigned> initial_results = runtime(s_ops, a_prms, output1, initial_populations, empty_virions, no_influx);
    std::cout << "Finished initial 1,000,000-timestep run\n";

    }

    // Scan existing file to get evolved rates
    std::string filename = "00_experiment_run__CA.txt";
    auto data_table = scan_file(filename);
    double lysTot = calculate_mean_last_n_rows(data_table, "lysTot", 5000);
    double lysgTot = calculate_mean_last_n_rows(data_table, "lysgTot", 5000);

    if (lysgTot > 0.0) a_prms["lysogenization_rate"] = lysgTot;
    if (lysgTot > 0.0) a_prms["lysogenization_rate_001"] = lysgTot;
    if (lysTot > 0.0) a_prms["lysis_rate"] = lysTot;
    if (lysTot > 0.0) a_prms["lysis_rate_001"] = lysTot;
    ...
    std::cout << "Start 3,000,000-timestep run with evolve_parameters_switch = true\n";
    runtime(s_ops, a_prms, output2, initial_populations, empty_virions, experimental_influx);
    std::cout << "Finished 3,000,000-timestep run\n";
    }
