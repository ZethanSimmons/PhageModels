          ...
          if (cur_cell->is_alive()) {
            ...
            } else if (event_chance < (influx_rate + reproduction_threshold()) && event_chance > reproduction_threshold()) 
            {
              ...
              if (evolve_parameters_switch)
              {
                ...
                if (cur_cell->is_lysogen() && mean_evolved_lysogenization_tot > 0.0 && mean_evolved_lysis_tot > 0.0) 
                {
                  cur_cell->set_lysogenization_rate(mean_evolved_lysogenization_tot);
                  cur_cell->set_lysis_rate(mean_evolved_lysis_tot);
                  
                  mutate_background_lysis(cur_cell);
                  mutate_lysogenization(cur_cell);

                }
                if (cur_cell->has_plasmid() && mean_evolved_conjugation_tot > 0.0) {
                  cur_cell->set_plasmid_conjugation_rate(mean_evolved_conjugation_tot);
                  mutate_conjugation(cur_cell);
                }
              }
            }
