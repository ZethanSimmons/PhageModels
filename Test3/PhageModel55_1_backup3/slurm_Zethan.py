...
"""
MEMO: This simulation checks the stability of resistance on phages and plasmids while influxing all types of bacteria and evolving lysis, lysogenization, and conjugation. Specifically, this simulation checks the impact of using 0 prophage loss rate instead of 0.00015, at the high death rate of 0.12
"""


home = os.getenv("HOME")

needed_files = [home + '/Thesis/PhageModel45_1_evolve_lysis_lysogenization_conjugation_phage_stability_check_all_invaders_CORRECTED2/demo', home + '/Thesis/PhageModel45_1_evolve_lysis_lysogenization_conjugation_phage_stability_check_all_invaders_CORRECTED2/source.tar.gz', home + '/Thesis/PhageModel45_1_evolve_lysis_lysogenization_conjugation_phage_stability_check_all_invaders_CORRECTED2/slurm_Zethan.py']

def main():
    base_dir = '/nesi/nobackup/uoa04131/data/PhageModel/PhageModel55_1_backup3/'

    command = """#!/bin/bash -e

#SBATCH --job-name=551b3_{pops:}      # job name (shows up in the queue)
...
srun demo --population {pops:} --s_antibiotic_time_periodicity {ab_period:} --s_antibiotic_time_limit {ab_lim:} --s_the_seed {seed:} --a_antibiotics {antibiotics:} --a_v_resistance {vres:} --a_p_resistance {pres:} --a_b_resistance {bres:} --a_lysogenization_rate {lysg:} --a_lysogenization_rate_001 {lysg1:} --a_lysis_rate {lys:} --a_lysis_rate_001 {lys1:} --a_conjugation {conj:} --a_conjugation_010 {conj1:} --s_random_mixing {rand_mix:} --s_death_lysis_switch {dlys:} --a_b_death {deth:} --a_v_diffusion {v_diffus:} --s_run1 {run1:} --s_run2 {run2:} --a_v_loss_rate 0

"""
    #Note that all influx rates have already been set in main.cpp, due to the more convenient interface there
    populations = (111, 119, 191, 911)
    ...
    death_rates = (0.12,) #no exploration of 0.02 or 0.06 for this test
