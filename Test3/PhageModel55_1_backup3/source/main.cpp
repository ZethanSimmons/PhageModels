...
{   
    ...
    0.000001, 0.000001, 0.000001, 0.000001, 0.000001, 0.000001, //susceptible phage
    0.000001, 0.000001, 0.000001, 0.000001, 0.000001, 0.000001  //AB-resistant phage
    }; 
    ...
    std::string loc1 = output_location + "/00_experiment_run_vloss0";
    std::string loc2 = output_location + "/02_experiment_run_evolved_vloss0";
    ...
    if (s_ops["run1"])
    {
    // Step 1: Setup first 1,000,000-timestep run with no influx, to evolve parameters
    ...
    double conjTot = calculate_mean_last_n_rows(data_table, "conjTot", 5000);
    ...
    if (conjTot > 0.0) a_prms["conjugation"] = conjTot;
    if (conjTot > 0.0) a_prms["conjugation_010"] = conjTot;
